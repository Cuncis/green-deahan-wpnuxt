...
<template>
    ...
    <section class="pt-28 pb-10 px-6 max-w-6xl mx-auto">
        <div class="reveal">
            <div class="flex items-center gap-2 text-xs font-semibold text-stone-400 mb-4">
                ...
                <span>›</span>
                <span class="text-[#006400]">Blog & Tips</span>
            </div>
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <span class="inline-block bg-green-100 text-[#006400] text-xs font-bold px-3 py-1.5 rounded-full mb-3 tracking-wide">📝 Tips & Panduan</span>
    ...
    <section v-if="status === 'pending'" class="px-6 max-w-6xl mx-auto py-16 text-center">
        <div class="inline-block w-10 h-10 border-4 border-[#006400] border-t-transparent rounded-full animate-spin mb-4"></div>
    ...
    <section v-else class="px-6 max-w-6xl mx-auto pb-16">
        ...
        <div v-if="filteredPosts.length > 0 && !searchQuery && activeCategory === 'semua'"
            class="mb-12 reveal">
                ...
                <div class="grid md:grid-cols-2">
                    ...
                    <div class="p-8 flex flex-col justify-center">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="bg-[#006400] text-white text-xs font-bold px-3 py-1 rounded-full">⭐ Artikel Pilihan</span>
                        ...
                        <h2 class="font-display font-black text-2xl md:text-3xl text-stone-900 group-hover:text-[#006400] transition-colors leading-tight mb-3">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-[#006400] font-bold text-sm group-hover:underline">Baca selengkapnya →</span>
                        </div>
                    </div>
                </div>
            </NuxtLink>
        ...
        <div v-if="filteredPosts.length > 0" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <NuxtLink v-for="(post, idx) in (searchQuery || activeCategory !== 'semua' ? filteredPosts : filteredPosts.slice(1))"
                :key="post.databaseId ?? idx"
                :to="post.uri ?? ('/' + post.slug)"
                :class="[
                ...
                <div class="relative overflow-hidden aspect-[16/9]">
                    <img v-if="post.featuredImage?.node?.sourceUrl"
                ...
                <div class="p-5">
                    ...
                    <h3 class="card-title font-display font-black text-base text-stone-900 leading-tight mb-2 line-clamp-2 transition-colors">
                        {{ post.title }}
                    </h3>
    ...
    <section class="py-16 px-6 bg-[#006400]">
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
            <div>
                <h2 class="font-display font-black text-2xl md:text-3xl text-white mb-2">Siap Membangun Lapangan Impian?</h2>
...
.blog-card:hover .card-title { color: #006400; }
